...
Reads any webpage aloud with the Kokoro TTS model running **fully locally in the
browser** (quantized q8, WASM — no server, no API keys). Highlights the sentence
...
2. First run downloads the voice model (~90 MB, cached by the browser after that)
...
- `offscreen document` — runs Kokoro (kokoro-js, WASM) and plays the audio,
